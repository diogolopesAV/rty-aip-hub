...
**External plugins** (submoduled into `plugins/`, full native UI support)
...
External repos with a complete plugin structure (`.claude-plugin/`, `.cursor-plugin/`, `skills/<name>/SKILL.md`) should be added as **git submodules** under `plugins/`. This makes them local paths, which is required for Cursor marketplace compatibility:

```bash
git submodule add https://github.com/org/repo plugins/repo-name
cd plugins/repo-name && git checkout <commit-sha>
cd ../.. && git add plugins/repo-name .gitmodules
```
...
├── .gitmodules                      # Git submodule pointers for external plugins
├── .claude-plugin/
...
└── plugins/
    ...
    └── azure-skills/                # ↳ submodule: microsoft/azure-skills (26 Azure skills)
```
